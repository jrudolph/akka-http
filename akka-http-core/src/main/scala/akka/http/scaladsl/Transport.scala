...
object Transport {
  def TCP(localAddress: Option[InetSocketAddress], settings: ClientConnectionSettings): Transport =
    new TCPTransport(localAddress, settings)

  private case class TCPTransport(localAddress: Option[InetSocketAddress], settings: ClientConnectionSettings) extends Transport {
    def connectTo(host: String, port: Int)(implicit system: ActorSystem): Flow[ByteString, ByteString, Future[OutgoingConnection]] =
