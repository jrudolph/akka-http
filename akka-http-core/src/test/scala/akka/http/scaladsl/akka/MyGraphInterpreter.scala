...
@InternalApi private[akka] final class GraphInterpreter(
  ...
  private[this] val ChaseLimit = 0 // 16 //if (fuzzingMode) 0 else 16
  ...
  private def processEvent(connection: Connection): Unit = {
    ...
    if ((code & (Pushing | InClosed | OutClosed)) == Pushing) {
      ...
      try connection.outHandler.onDownstreamFinish() finally activeStage.isRunning = false
      ...
      if ((code & Pushing) == 0) {
        ...
        finally activeStage.isRunning = false
      } else {
  ...
  private def processPush(connection: Connection): Unit = {
    ...
    try connection.inHandler.onPush() finally activeStage.isRunning = false
  }
  ...
  private def processPull(connection: Connection): Unit = {
    ...
    try connection.outHandler.onPull() finally activeStage.isRunning = false
  }
  ...
  private[stream] def chasePush(connection: Connection): Unit = {
    if (chaseCounter > 0 && !connection.inOwner.isRunning /*&& chasedPush == NoEvent*/ ) {
      chaseCounter -= 1
      val oldActive = activeStage
      // FIXME: do directly if allowed
  ...
  private[stream] def chasePull(connection: Connection): Unit = {
    if (chaseCounter > 0 && !connection.outOwner.isRunning /*&& chasedPull == NoEvent*/ ) {
      chaseCounter -= 1
      val oldActive = activeStage
      // FIXME: do directly if allowed
