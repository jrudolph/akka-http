...
class RendererSpec extends AnyWordSpec with Matchers {
  ...
  "The request/response common header logic" should {
    ...
    "output headers" in {
      ...
      Renderer.renderHeaders(headers, builder, None, NoLogging, isServer = true)
    ...
    "add a date header when none is present" in {
      val builder = new VectorBuilder[(String, String)]
      Renderer.renderHeaders(Seq.empty, builder, None, NoLogging, isServer = true)
    ...
    "keep the date header if it already is present" in {
      ...
      Renderer.renderHeaders(Seq(Date(originalDateTime)), builder, None, NoLogging, isServer = true)
      val date = builder.result().collectFirst {
        case ("date", str) => str
      }
    ...
    "add server header if default provided (in server mode)" in {
      val builder = new VectorBuilder[(String, String)]
      Renderer.renderHeaders(Seq.empty, builder, Some(("server", "default server")), NoLogging, isServer = true)
    ...
    "keep server header if explicitly provided (in server mode)" in {
      val builder = new VectorBuilder[(String, String)]
      Renderer.renderHeaders(Seq(Server("explicit server")), builder, Some(("server", "default server")), NoLogging, isServer = true)
      val result = builder.result().find(_._1 == "server").map(_._2)
    ...
    "add user-agent header if default provided (in client mode)" in {
      val builder = new VectorBuilder[(String, String)]
      Renderer.renderHeaders(Seq.empty, builder, Some(("user-agent", "fancy browser")), NoLogging, isServer = false)
    ...
    "keep user-agent header if explicitly provided (in server mode)" in {
      val builder = new VectorBuilder[(String, String)]
      Renderer.renderHeaders(Seq(`User-Agent`("fancier client")), builder, Some(("user-agent", "fancy browser")), NoLogging, isServer = false)
      val result = builder.result().find(_._1 == "user-agent").map(_._2)
    ...
    "exclude explicit headers that is not valid for HTTP/2" in {
      ...
      Renderer.renderHeaders(invalidExplicitHeaders, builder, None, NoLogging, isServer = true)
    ...
    "exclude headers that should not be rendered in responses" in {
      ...
      Renderer.renderHeaders(shouldNotBeRendered, builder, None, NoLogging, isServer = true)
      builder.result().exists(_._1 != "date") shouldBe false
    ...
    "exclude explicit raw headers that is not valid for HTTP/2 or should not be provided as raw headers" in {
      ...
      Renderer.renderHeaders(invalidRawHeaders, builder, None, NoLogging, isServer = true)
      builder.result().exists(_._1 != "date") shouldBe false
    }
